# -*- coding: utf-8 -*-
"""
Created on Mon Sep 19 11:11:59 2022
...
def replace_with_mean(arr):
  # Check if input array is 3D
  if arr.ndim != 3:
    raise ValueError("Input array must be 3D")

  # Get shape of array
  nwave, nx, ny = arr.shape

  # Calculate mean value of array
  mean_val = np.mean(arr)

  # Create output array with same shape as input array
  arr_mean = np.empty_like(arr)

  # Iterate over elements of input array
  for i in range(nwave):
    for j in range(nx):
      for k in range(ny):
        # If element is negative or NaN, set to mean value
        if arr[i,j,k] < 0 or np.isnan(arr[i,j,k]):
          arr_mean[i,j,k] = mean_val
        else:
          # Otherwise, copy value from input array to output array
          arr_mean[i,j,k] = arr[i,j,k]
...
#map_flux0 = replace_with_mean(map_flux0)                                 # structure: (wave,y,x)


map_var = np.sqrt(cube[2].data)                         # variance
#map_var = cube[1].data                                 # structure: (wave,y,x)
#map_var = replace_with_mean(map_var)
#map_var = np.where(np.logical_or(np.isnan(map_var), map_var < 0), np.nan, map_var)
#nans1, x1= nan_helper(map_var)
#map_var[nans1]= np.interp(x1(nans1), x1(~nans1), map_var[~nans1])
...
pixsize_califa = 0.2							# in erg/s/cm^2/Ang.
# get dimension
...
#		rows, cols = np.where((map_flux0[bb]>0))
#		mean = np.mean(map_flux0[bb])
#
#		map_flux0[bb] = np.where(map_flux0[bb] == 0, mean, map_flux0[bb])
#                #map_flux0[bb][rows,cols] = lowest
#                #rows1,cols1=np.where(map_flux0[bb])
#for bb in range(0,nwaves):
#		rows, cols = np.where((map_var[bb]>0))
#		mean = np.mean(map_var[bb])
#
#                #rows, cols = np.where((map_var[bb]<0),np.isnan(map_flux0[bb]))
#		map_var[bb] = np.where(map_var[bb] == 0, mean, map_var[bb])
if spec_smoothing==1:
	...
	map_flux = map_flux0
                ...
                #map_var[bb][rows,cols] = lowest
# get kernel for PSF matching
# All the kernels were brought to 0.25"/pixel sampling
dir_file = PIXEDFIT_HOME+'/data/kernels/'
kernel_name = 'Kernel_HiRes_Moffet_02.5_to_WISE_ATLAS_3.4.fits' #% filter_ref_psfmatch  #kernel_califa_to_%s.fits.gz
...
for ii in recvbuf_idx:
	...
	data_image = layer_ifu_flux/pixsize_califa/pixsize_califa				# surface brightness
	align_psfmatch_layer_ifu_flux, footprint = reproject_exact((data_image,header_stamp_image),header_califa2D )
	align_psfmatch_layer_ifu_flux = align_psfmatch_layer_ifu_flux*pixsize_image*pixsize_image

	data_image = layer_ifu_var/pixsize_califa/pixsize_califa
	align_psfmatch_layer_ifu_var, footprint = reproject_exact((data_image,header_stamp_image),header_califa2D )
