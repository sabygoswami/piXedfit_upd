...
try:
	...
	PIXEDFIT_HOME = '/home/saby/piXedfit'   #os.environ['PIXEDFIT_HOME']
...
def specphoto_califagalexsdss2masswise(photo_fluxmap, califa_file, spec_smoothing=False, kernel_sigma=2.6, 
	...
	#os.system('move %s %s' % (name_config,temp_dir))
	...
	os.system("python3 %s./sp_clf.py %s" % (CODE_dir,name_config)) #mpirun -n %d nproc_new,
    #os.system("mpirun -n %d python3 %s./sp_clf.py %s" % (nproc_new,CODE_dir,name_config))
...
def match_imgifs_spectral(specphoto_file, models_spec=None, nproc=10, del_wave_nebem=10.0, cosmo=0, 
	...
	os.system("mpirun -n %d python3 %s./mtch_sph_fnal.py %s" % (nproc,CODE_dir,name_config)) #mpirun -n %d  nproc,
